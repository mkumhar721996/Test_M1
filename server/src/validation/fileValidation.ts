...
export function validateIdProof(file: UploadedFileMeta): string | null {
  if (!ID_PROOF_ALLOWED_MIME_TYPES.includes(file.mimeType)) {
    return `ID proof must be a PDF, JPG, or PNG file. "${file.originalName}" was rejected.`;
  }
