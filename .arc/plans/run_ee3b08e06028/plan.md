summary: |
  ...
  Testing Library tests against the prototype's exact copy and DOM structure). Beyond the 12
  literal ACs, this revision adds explicit edge-case coverage the reviewer asked for: whitespace-
  only "required" fields, malformed email/mobile variants beyond the single example in each AC,
  a defensive department allow-list check, name trimming on storage, a genuinely-omitted optional
  file vs. an invalid one, list ordering on GET /api/employees, and a same-email concurrency race
  (two simultaneous submissions for the identical new address, not just two different ones).
scope:
  - description: |
  ...
  - description: |
      ...
      prototype's own client-side validation strings (design HTML lines 1256-1273). Beyond the
      single-example ACs, this module also treats a whitespace-only value ("   ") as blank (not
      just an empty string), and adds a defensive allow-list check on `department` against the
      exact nine options in the prototype's `<select>` (design HTML lines 963-972) — the UI can
      only ever submit one of those values, but a direct API call is not so constrained, so the
      backend re-checks it rather than trusting the client's `<select>`.
      ```ts
      const ALLOWED_DEPARTMENTS = [
        'Engineering', 'Sales', 'Marketing', 'Finance', 'HR',
        'Operations', 'IT', 'Customer Support', 'Legal',
      ];
    ...
    rationale: |
      ...
      must never trust the client, plus the whitespace-only-field and department-allow-list edge
      cases called out above. Message text is copied from the prototype so the UI-layer test
      (scope item 13) can assert on the identical strings.
  ...
  - description: |
      ...
      (design HTML lines 1129-1184). Both functions are only ever called when a file was
      actually selected — an omitted optional file must short-circuit before reaching this
      module (see the service in scope item 6), so "no file chosen" and "an invalid file chosen"
  ...
  - description: |
      ...
      library, and is also what prevents the harder edge case of two concurrent submissions for
      the *same* brand-new email both succeeding (see the added race test in `tests`). Name
      fields are trimmed on the way in, mirroring the trim already applied to email, so
      "  Meera  " is stored as "Meera" rather than leaking incidental whitespace into the record.
      ```ts
    ...
    rationale: |
      ...
      insensitive via `.trim().toLowerCase()`), and AC12 (atomic sequential assignment), plus the
      name-trimming and same-email-race edge cases.

  - description: |
      Employee service that orchestrates field validation, file validation, and repository
      creation for a single registration request. A file is only passed to `validatePhoto`/
      `validateIdProof` when it is present on the input — an absent optional file skips straight
      to `photo: null` / `idProof: null` and can never itself produce a validation error, which
      is what keeps AC5 (blank optional fields succeed) and AC9/AC10 (an invalid *selected* file
      is rejected) from colliding.
      ```ts
    ...
    rationale: |
      ...
      (optional fields blank), AC6 (optional fields provided and persisted), and the
      omitted-vs-invalid-file edge case.
  ...
  - description: |
      ...
      listing, backing the approved Employee Directory screen, returned in creation order so a
      newly registered employee is visibly appended rather than reordering existing rows). An
      error-handling middleware maps `ValidationError` to `400 { error: 'VALIDATION_ERROR',
      fieldErrors }` and `DuplicateEmailError` to `409 { error: 'DUPLICATE_EMAIL', message,
      existingEmployeeId }`.
    files:
    ...
    rationale: |
      ...
      below), the same-email race edge case, and the GET-ordering edge case, and is what the
      frontend's API client actually calls.
  ...
  - description: |
      Client-side validation mirroring the backend's copy exactly (same regexes, whitespace-only
      handling, and department allow-list as scope item 3), used for the inline, pre-submit
      feedback the ACs require.
    files:
  ...
  - description: |
      ...
      line 1325). Submitting with several required fields blank at once must surface an error
      under every one of them simultaneously, not just the first, matching the prototype's own
      `requiredMissing.forEach(...)` loop at design HTML lines 1256-1258.
    files:
      - web/src/pages/RegisterEmployee.tsx
      - web/src/pages/RegisterEmployee.test.tsx
    rationale: |
      This is the primary UI surface for AC1, AC4-AC11, plus the multi-field-blank and
      malformed-format edge cases.
...
tests:
  ...
  - |
    ...
    ```
  - |
    AC7 edge case — a whitespace-only value ("   ") in a required field is treated as blank, not
    as a filled-in value (server/test/employeeValidation.test.ts):
    ```ts
    expect(validateEmployeeFields({ ...valid, firstName: '   ' }).firstName)
      .toBe('First name is required.');
    ```
  - |
    AC7 edge case — leaving several required fields blank at once surfaces an error under every
    one of them simultaneously, not just the first (web/src/pages/RegisterEmployee.test.tsx):
    ```tsx
    render(<RegisterEmployee />);
    await userEvent.click(screen.getByRole('button', { name: /register employee/i }));
    expect(screen.getByText('First name is required.')).toBeInTheDocument();
  ...
  - |
    ...
    ```
  - |
    AC8 edge case — mobile numbers that are the wrong length or contain non-digit characters
    (dashes, spaces, a country-code "+91" prefix) are all rejected with the same message
    (server/test/employeeValidation.test.ts):
    ```ts
    expect(validateEmployeeFields({ ...valid, mobile: '987654321' }).mobile)
      .toBe('Mobile number must be exactly 10 numeric digits.');
    expect(validateEmployeeFields({ ...valid, mobile: '98765-4321' }).mobile)
      .toBe('Mobile number must be exactly 10 numeric digits.');
    expect(validateEmployeeFields({ ...valid, mobile: '+919876543210' }).mobile)
      .toBe('Mobile number must be exactly 10 numeric digits.');
    ```
  - |
    AC8 edge case — email addresses missing a domain/TLD or containing a double "@" are rejected
    (server/test/employeeValidation.test.ts):
    ```ts
    ...
    ```
  - |
    Edge case (defensive, design-driven, not a literal AC) — a `department` value outside the
    prototype's fixed nine-option `<select>` list (design HTML lines 963-972) is rejected even
    though it is non-blank, since an API caller is not constrained to the UI's own options
    (server/test/employeeValidation.test.ts):
    ```ts
    expect(validateEmployeeFields({ ...valid, department: 'Not A Real Dept' }).department)
  ...
  - |
    ...
    ```
  - |
    AC9/AC10 edge case — genuinely omitting the optional Photo/ID Proof file entirely (no
    `.attach()` call at all) must NOT trigger a file-type or file-size error; only a selected-
    and-invalid file does (server/test/employees.route.test.ts):
    ```ts
    const res = await createEmployee(app, { /* no photo/idProof attach() at all */ });
    expect(res.status).toBe(201);
    expect(res.body.photo).toBeNull();
    ```
  - |
    ...
    ```
  - |
    AC1/AC6 edge case — leading/trailing whitespace in First/Last name is trimmed before storage
    (server/test/employeeRepository.test.ts):
    ```ts
    ...
    ```
  - |
    AC12 — backend HTTP integration test, two concurrent valid submissions with two different
    emails each get a distinct, sequential Employee ID:
    ```ts
    ...
    ```
  - |
    AC4/AC12 combined edge case — two concurrent submissions using the SAME brand-new email
    (a race, not just two different emails) result in exactly one success and one duplicate
    rejection, never two successes and never two rejections
    (server/test/employees.route.test.ts):
    ```ts
    const [r1, r2] = await Promise.all([
      createEmployee(app, { email: 'race@x.com' }),
      createEmployee(app, { email: 'race@x.com' }),
    ]);
    ...
    ```
  - |
    Directory-ordering edge case (supports AC1/AC3 visibility on the approved Employee Directory
    screen) — GET /api/employees returns records in creation order so a newly registered
    employee is appended, not reordered (server/test/employees.route.test.ts):
    ```ts
    await createEmployee(app, { email: 'a@x.com' });
    await createEmployee(app, { email: 'b@x.com' });
    const res = await request(app).get('/api/employees');
    expect(res.body.map((e: any) => e.email)).toEqual(['a@x.com', 'b@x.com']);
    ```
assumptions_or_open_questions:
  ...
  - |
    ...
    deployment topology exists in this repo. The added same-email race test exercises the harder
    variant of this same mechanism (duplicate-detection under concurrency, not just ID
    assignment) but is still an in-process guarantee only.
  - |
  ...
  - |
    The `department` allow-list check (the nine `<select>` options at design HTML lines 963-972)
    and the whitespace-only-required-field handling are added as defensive backend validation
    for edge cases the literal ACs don't name explicitly, since they follow directly from what
    the approved design's own form already constrains client-side. If the reviewer wants the
    backend to stay strictly scoped to the literal AC wording, this check can be dropped.
  - |
    The prototype sets `max` (today's date) on both the Joining Date and Date of Birth inputs
    (design HTML lines 1220-1222), implying future dates shouldn't be accepted, but no AC states
    this as a business rule. This plan does NOT add a server-side future-date rejection, since
    that would be inventing a rule beyond both the ACs and the design's own script (which only
    enforces it via the HTML `max` attribute, not in `validateEmployeeFields`). Flag if backend
    date-range enforcement is actually wanted.
package_dependencies:
  ...
  - name: supertest
    version: ^7.0.0
    ecosystem: npm
    rationale: Drives the HTTP-level tests against the Express app (AC1, AC4-AC6, AC12, and the added race/ordering edge cases) without binding a real port.
  - name: react
    version: ^18.3.1
    ecosystem: npm
...
notes: |
  ...
  - Edge-case coverage added in this revision (whitespace-only required fields, malformed
    email/mobile variants, department allow-list, name trimming, omitted-vs-invalid optional
    file, directory ordering, same-email concurrency race) targets failure modes the design's
    own script already anticipates (e.g. the `requiredMissing.forEach` multi-error loop at
    design HTML lines 1256-1258, the `.trim()` calls at lines 1237-1244) or that follow directly
    from the repository's synchronous check-and-insert (scope item 5), rather than inventing
    behavior the design/ACs don't support.
